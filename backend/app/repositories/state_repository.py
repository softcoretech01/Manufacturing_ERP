...
class StateRepository:
    ...
    async def get_next_code(self) -> Dict[str, str]:
        query = text("CALL SpGetNextStateCode()")
        result = await self.db.execute(query)
        row = result.fetchone()
        return {"nextCode": row.nextCode}

    async def get_all(self) -> List[Dict[str, Any]]:
        query = text("CALL SpState('LIST', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
        result = await self.db.execute(query)
        return [self._row_to_dict(row) for row in result.fetchall()]

    async def get_by_id(self, id: int) -> Dict[str, Any]:
        query = text("CALL SpState('READ', :id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
    ...
    async def create(self, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpState(
                'CREATE', NULL, :code, :name, :gstCode, :country, :stateType, :zone,
    ...
    async def update(self, id: int, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpState(
                'UPDATE', :id, :code, :name, :gstCode, :country, :stateType, :zone,
                :status, :effectiveFrom, :effectiveTo, :modifiedBy
            )
        """)
        params = {
    ...
    async def delete(self, id: int, current_user: str) -> None:
        query = text("CALL SpState('DELETE', :id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, :modifiedBy)")
