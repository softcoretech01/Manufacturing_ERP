...
class TaxRepository:
    ...
    async def get_next_code(self) -> Dict[str, str]:
        query = text("CALL SpGetNextTaxCode()")
        result = await self.db.execute(query)
        row = result.fetchone()
        return {"nextCode": row.nextCode}

    async def get_all(self) -> List[Dict[str, Any]]:
        query = text("CALL SpTax('LIST', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
        result = await self.db.execute(query)
        return [self._row_to_dict(row) for row in result.fetchall()]

    async def get_by_id(self, id: int) -> Dict[str, Any]:
        query = text("CALL SpTax('READ', :id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
    ...
    async def create(self, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpTax(
                'CREATE', NULL, :code, :name, :taxType, :rate,
                :appliesTo, :isReverseCharge, :glAccount,
        ...
        params = {
            ...
            'isReverseCharge': data.get('isReverseCharge', False),
            'glAccount': data.get('glAccount'),
    ...
    async def update(self, id: int, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpTax(
                'UPDATE', :id, :code, :name, :taxType, :rate,
                :appliesTo, :isReverseCharge, :glAccount,
                :status, :effectiveFrom, :effectiveTo, :revision, :usageCount, :modifiedBy
            )
        """)
