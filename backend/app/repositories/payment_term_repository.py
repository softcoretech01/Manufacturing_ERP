...
class PaymentTermRepository:
    ...
    def _row_to_dict(self, row) -> Dict[str, Any]:
        ...
        for column in row._mapping.keys():
            ...
            elif column == 'DiscountPct' and val is not None:
    ...
    async def get_next_code(self) -> Dict[str, str]:
        query = text("CALL SpGetNextPaymentTermCode()")
        result = await self.db.execute(query)
        row = result.fetchone()
        return {"nextCode": row.nextCode}

    async def get_all(self) -> List[Dict[str, Any]]:
        query = text("CALL SpPaymentTerm('LIST', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
        result = await self.db.execute(query)
        return [self._row_to_dict(row) for row in result.fetchall()]

    async def get_by_id(self, id: int) -> Dict[str, Any]:
        query = text("CALL SpPaymentTerm('READ', :id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
    ...
    async def create(self, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpPaymentTerm(
                'CREATE', NULL, :code, :name, :creditDays, :baseDate,
                :discountPct, :discountDays, :appliesTo,
                :status, :effectiveFrom, :effectiveTo, :revision, :usageCount, :modifiedBy
            )
        """)
        params = {
            'code': data.get('code'),
            ...
            'discountDays': data.get('discountDays', 0),
    ...
    async def update(self, id: int, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpPaymentTerm(
                'UPDATE', :id, :code, :name, :creditDays, :baseDate,
                :discountPct, :discountDays, :appliesTo,
                :status, :effectiveFrom, :effectiveTo, :revision, :usageCount, :modifiedBy
            )
        """)
    ...
    async def delete(self, id: int, current_user: str) -> None:
        query = text("CALL SpPaymentTerm('DELETE', :id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, :modifiedBy)")
