...
class BottleColourRepository:
    ...
    async def get_next_code(self) -> Dict[str, str]:
        query = text("CALL SpGetNextBottleColourCode()")
        result = await self.db.execute(query)
        row = result.fetchone()
    ...
    async def create(self, data: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        query = text("""
            CALL SpBottleColour(
                'CREATE', NULL, :p_Code, :p_Name, :p_Hex, :p_RalCode, :p_Finish, :p_Process, 
                :p_Consumable, :p_Status, :p_EffectiveFrom, :p_EffectiveTo, 
        ...
        params = {
            ...
            "p_Hex": data.get("hex"),
            "p_RalCode": data.get("ralCode"),
            "p_Finish": data.get("finish"),
            "p_Process": data.get("process"),
            "p_Consumable": data.get("consumable"),
    ...
    async def update(self, record_id: int, data: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        query = text("""
            CALL SpBottleColour(
                'UPDATE', :p_Id, NULL, :p_Name, :p_Hex, :p_RalCode, :p_Finish, :p_Process, 
                :p_Consumable, :p_Status, :p_EffectiveFrom, :p_EffectiveTo, 
                :p_Revision, :p_ModifiedBy
            )
        """)
