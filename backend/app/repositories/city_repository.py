...
class CityRepository:
    ...
    async def get_next_code(self) -> Dict[str, str]:
        query = text("CALL SpGetNextCityCode()")
        result = await self.db.execute(query)
        row = result.fetchone()
        return {"nextCode": row.nextCode}

    async def get_all(self) -> List[Dict[str, Any]]:
        query = text("CALL SpCity('LIST', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
        result = await self.db.execute(query)
        return [self._row_to_dict(row) for row in result.fetchall()]

    async def get_by_id(self, id: int) -> Dict[str, Any]:
        query = text("CALL SpCity('READ', :id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)")
    ...
    async def create(self, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpCity(
                'CREATE', NULL, :code, :name, :state, :pincodeRange, :deliveryZone, :transitDays,
                :isPort, :status, :effectiveFrom, :effectiveTo, :modifiedBy
            )
        """)
        params = {
            'code': data.get('code'),
            ...
            'pincodeRange': data.get('pincodeRange'),
            'deliveryZone': data.get('deliveryZone'),
            'transitDays': data.get('transitDays', 0),
            'isPort': data.get('isPort', False),
    ...
    async def update(self, id: int, data: Dict[str, Any], current_user: str) -> Dict[str, Any]:
        query = text("""
            CALL SpCity(
                'UPDATE', :id, :code, :name, :state, :pincodeRange, :deliveryZone, :transitDays,
                :isPort, :status, :effectiveFrom, :effectiveTo, :modifiedBy
            )
        """)
        params = {
    ...
    async def delete(self, id: int, current_user: str) -> None:
        query = text("CALL SpCity('DELETE', :id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, :modifiedBy)")
