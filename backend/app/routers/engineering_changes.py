...
from app.core.deps import get_context, require
...
@router.get("", response_model=List[EngChangeSchema], dependencies=[Depends(require("ENGINEERING.CHANGE.VIEW"))])
async def get_changes(service: EngineeringChangeService = Depends(get_service)):
    return await service.get_all_changes()

@router.get("/next-code", dependencies=[Depends(require("ENGINEERING.CHANGE.CREATE"))])
...
async def create_change(
    ...
    user_id = ctx.login_id or "system"
    ...
    await log_audit_entry(
        ...
        document_no=result.get("docNo"),
        action="CREATE",
        changes=[{"field": "all", "old": None, "new": "Created"}]
    )
    return result

@router.put("/{change_uid}", response_model=EngChangeSchema, dependencies=[Depends(require("ENGINEERING.CHANGE.EDIT"))])
...
@router.delete("/{change_uid}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require("ENGINEERING.CHANGE.DELETE"))])
