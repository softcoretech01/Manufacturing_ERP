...
@router.get("", response_model=List[EngToolSchema], dependencies=[Depends(require("ENGINEERING.TOOL.VIEW"))])
async def get_tools(service: EngineeringToolService = Depends(get_service)):
    return await service.get_all_tools()

@router.get("/next-code", dependencies=[Depends(require("ENGINEERING.TOOL.CREATE"))])
...
@router.post("", response_model=EngToolSchema, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require("ENGINEERING.TOOL.CREATE"))])
...
@router.put("/{tl_uid}", response_model=EngToolSchema, dependencies=[Depends(require("ENGINEERING.TOOL.EDIT"))])
...
@router.delete("/{tl_uid}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require("ENGINEERING.TOOL.DELETE"))])
