...
function splitActions(node: ReactNode) {
  ...
  const destructive = remove ?? all.find((el) => el.props.danger && el !== edit)
  const rest = all.filter((el) => el !== edit && el !== destructive)
  return { edit, destructive, isDelete: !!remove, rest }
}
  ...
  return (
    <div className="flex items-center justify-end gap-0.5">
      ...
      {rest.length > 0 && (
        <Menu
          trigger={
            <Button variant="ghost" size="icon-sm" className="h-6 w-6" title="More actions">
