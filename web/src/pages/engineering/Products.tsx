...
export function ProductsPage() {
  ...
  const columns: Column<EngProduct>[] = [
    ...
    { key: 'productType', header: 'Type', sortable: true, width: '9rem', accessor: (p) => TYPE_LABEL[p.productType], render: (p) => <span className="text-xs text-fg-muted">{TYPE_LABEL[p.productType]}</span> },
    { key: 'capacityMl', header: 'Capacity', align: 'right', sortable: true, width: '6.5rem', accessor: (p) => p.capacityMl ?? 0, render: (p) => (p.capacityMl ? `${p.capacityMl} ml` : '—') },
    { key: 'materialGrade', header: 'Grade', width: '6rem', accessor: (p) => p.spec.materialGrade, render: (p) => <span className="text-xs text-fg-muted">{p.spec.materialGrade}</span> },
    { key: 'revision', header: 'Rev', align: 'right', width: '4.5rem', accessor: (p) => p.revision, render: (p) => <span className="font-mono text-2xs">R{p.revision}</span> },
    {
      ...
      header: 'BOM',
      width: '9rem',
      accessor: (p) => defaultBomFor(p.code, boms)?.docNo ?? '',
      render: (p) => {
        const b = defaultBomFor(p.code, boms)
        return b ? <span className="font-mono text-2xs text-fg-muted">{b.docNo} R{b.revision}</span> : <span className="text-2xs text-danger">none</span>
      },
    },
    {
      key: 'routing',
      header: 'Routing',
      width: '9rem',
      accessor: (p) => defaultRoutingFor(p.code, routings)?.docNo ?? '',
      render: (p) => {
        const r = defaultRoutingFor(p.code, routings)
        return r ? <span className="font-mono text-2xs text-fg-muted">{r.docNo} R{r.revision}</span> : <span className="text-2xs text-danger">none</span>
      },
    },
    { key: 'standardCost', header: 'Standard cost', align: 'right', sortable: true, width: '9rem', accessor: (p) => p.standardCost, render: (p) => (p.standardCost > 0 ? `₹${formatAmount(p.standardCost)}` : <span className="text-2xs text-fg-subtle">not rolled</span>) },
  ...
  return (
    <div>
        ...
        tabs={
          <Tabs
            ...
            tabs={[
              ...
              { id: 'production', label: 'In production', count: counts.production },
      ...
      <DataTable
        ...
        rowActions={(p) => (
          <>
            <MenuItem label="Open" onClick={() => { setDetail(p); setDetailTab('general') }} />
