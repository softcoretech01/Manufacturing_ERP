...
export function MasterOverviewPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Master data"
        description="Every transaction in the ERP points at a master record. This is where they live, and where their quality is watched."
        breadcrumbs={[{ label: 'Home', to: '/masters' }, { label: 'Master data' }]}
        actions={
          <>
            <Link to="/masters/import">
              <Button variant="outline" size="sm" icon={<FileSpreadsheet className="h-4 w-4" />}>
                Import &amp; export
              </Button>
            </Link>
            <Link to="/masters/duplicates">
              <Button variant="primary" size="sm" icon={<Copy className="h-4 w-4" />}>
                Duplicate review
              </Button>
            </Link>
          </>
        ...
        tabs={
          <Tabs
            ...
            onChange={setTab}
            tabs={[
              { id: 'catalogue', label: 'Catalogue', count: all.length },
      ...
      {openImports.length > 0 && (
        <Alert
          tone="warning"
          className="mb-4"
          title={`${openImports.length} import awaiting a decision`}
          action={<Link to="/masters/import"><Button size="sm" variant="outline">Review</Button></Link>}
        >
          {openImports[0].fileName} — {openImports[0].rowsError} error rows and{' '}
          {openImports[0].rowsWarning} warnings from {openImports[0].rowsTotal}. Nothing is written
          until the dry run is accepted.
        </Alert>
      )}
