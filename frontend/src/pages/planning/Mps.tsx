...
export function MpsPage() {
  ...
  // `manufacturableProducts` is FINISHED and SEMI_FINISHED only. `products`
  // stays available for anything that needs the full item list.
  const { mps, demand, products, manufacturableProducts, ctx, starts } = usePlanningData()
  const { rows, create, update } = mps

  // Default to the first schedulable product. The old `lifecycle === 'PRODUCTION'`
  // test was no filter at all: every ACTIVE item maps to that lifecycle, so it
  // selected argon gas.
      ...
      <Card className="mb-4">
        <CardBody className="grid gap-3.5 sm:grid-cols-4">
          <Select
            ...
            options={[{ value: '', label: 'Select a product…' }, ...manufacturableProducts.rows.map((p) => ({ value: p.code, label: `${p.code} — ${p.name}` }))]}
          />
