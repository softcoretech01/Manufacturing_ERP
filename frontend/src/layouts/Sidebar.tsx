...
function NavRow({
  ...
  // Sub-group: has children but no direct route
    ...
    return (
      <div>
        <button
          ...
          className={cn(
            'group flex w-full items-center gap-2.5 rounded-lg px-3 py-1.5 text-left text-[13px] font-medium transition-colors duration-200',
            'text-[#5B6478] hover:bg-[#F5F8FF] hover:text-fg',
          )}
        >
        ...
        {subOpen && (
          <div className="mt-0.5 space-y-0.5 border-l border-border pl-3 ml-3.5">
            {item.children.map((child) => (
              <NavRow key={child.label} item={child} collapsed={false} nested badgeCount={undefined} />
            ))}
          </div>
        )}
      </div>
